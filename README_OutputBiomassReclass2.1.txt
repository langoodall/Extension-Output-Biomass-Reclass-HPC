...
Extension-Output-Biomass-Reclass uses cohort information at each site to classify every active site into 
new forest types which are amalgamations of various species. Forest types, as associations or group of species,
are arbitrary because they are completely user-defined. The reclassification algorithm uses a dominance value 
that is calculated for each species. The dominance value is equal to either 1) the total biomass (B) (Mg/ha) for 
the species or 2) [B * -1], as determined by the user. The species dominance values are summed for each user-defined 
forest type to which they are assigned. Note that some species may have a negative dominance for a given forest 
type, as determined by the user. A site is then assigned to (given the value of) the forest type with the highest 
total dominance value.
